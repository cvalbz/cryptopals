...
def md_padding(message_old):
    ...
    return message[len(message_old):]
...
def roll_cascade_function(state, fake_message, append):
    ...
    # add padding as if we are signing the extended message
    fake_padding = md_padding(fake_message + append)

    # but compute only the extension and padding
    message = append + fake_padding
    # Process the message in successive 512-bit chunks:
    # break message into 512-bit chunks
    for i in range(0, len(message), 64):
        w = [0] * 80
